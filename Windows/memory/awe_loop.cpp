...
int main() {
    ...
    const ULONG_PTR numberOfPages = 10000;
    const size_t totalSize = pageSize * numberOfPages;
    const size_t NUM_PAGES = 5000000;
    ...
    for (size_t i = 0; i < NUM_PAGES; ++i) {        
        ...
        if(strcmp((char*)pAWE, "AWE Memory Access Success!") != 0){
        ...
        if(strcmp((char*)pAWE, "AWE2 Memory Access Success!") != 0){
            DbgLog("Data in AWE: %s\n", pAWE);
            DbgLog("Loop %zu...\n", i);
        }
        if (i % 10000 == 0) DbgLog("Loop %zu...\n", i);
    }
    DebugBreak();
